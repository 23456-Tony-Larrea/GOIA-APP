...
const RoleView = ({ }) => {
  ...
  const [roleValido, setRoleValido] = useState(true);
  ...
  const createRole = async () => {
    try {
      ...
      if (idRole) {
        const validateUpdate = selectedName;
        if (validateUpdate === "") {
          console.log("El campo está vacío");
          setRoleValido(false);
        } else{
        const response = await axios.put(`/roles/${idRole}`, { name: validateUpdate});
        ...
        const validate = selectedName;
        if (validate === "") {
          console.log("El campo está vacío");
          setRoleValido(false);
        } else {
          try {
            const response = await axios.post('/roles', { name: validate });
            const createdRole = response.data;
            setRoles(prevRoles => [...prevRoles, createdRole]);
            setModalVisible(false);
            Dialog.show({
          ...
          <Modal visible={modalVisible} transparent animationType="fade">
            <View style={styles.modalContainer}>
              <View style={styles.modalContent}>
                ...
                />
                {!roleValido && <Text style={{ color: 'red' }}>El nombre de rol es requerido</Text>}
